...
def stemmer(tag_q1, tag_q2):
    ...
    for token in tag_q1:
        stem_q1.append((stem(token[0]), token[1]))

    for token in tag_q2:
        stem_q2.append((stem(token[0]), token[1]))
...
def semanticSimilarity(q1, q2):
    ...
    tag_q1, tag_q2 = tag(tokens_q1, tokens_q2)
    # stem_q1, stem_q2 = stemmer(tag_q1, tag_q2)
...
if __name__ == '__main__':
    ...
    for row in train_qs.itertuples():
        try:
            count += 1
            print semanticSimilarity(row[2], row[3]), row[4]
            # break
        except TypeError:
            pass
